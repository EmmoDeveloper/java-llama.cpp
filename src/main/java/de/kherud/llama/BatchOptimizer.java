...
public class BatchOptimizer {
	...
	public static ModelParameters applyBatchOptimization(ModelParameters params) {
		...
		if (!hasBatchConfig) {
			...
			System.out.println("🚀 Auto-optimized batch size: " + config.batchSize + 
				" (" + config.description + ")");
		}
		
		if (!hasUbatchConfig) {
			params.setUbatchSize(config.ubatchSize);
			System.out.println("⚡ Auto-optimized ubatch size: " + config.ubatchSize);
		}
	...
	public static void validateBatchConfiguration(ModelParameters params) {
		...
		if (batchStr != null && ubatchStr != null) {
			...
			if (ubatchSize > batchSize) {
				System.out.println("⚠️  Warning: ubatch size (" + ubatchSize + 
					") is larger than batch size (" + batchSize + ")");
				System.out.println("   Consider reducing ubatch size to " + batchSize + " or less");
			}
			
			// Suggest optimizations for very small or very large batch sizes
			if (batchSize < MIN_BATCH_SIZE) {
				System.out.println("💡 Tip: Batch size " + batchSize + " may be too small for optimal throughput");
				System.out.println("   Consider increasing to " + MIN_BATCH_SIZE + " or higher");
			} else if (batchSize > MAX_BATCH_SIZE) {
				System.out.println("💡 Tip: Batch size " + batchSize + " may cause memory issues");
				System.out.println("   Consider reducing to " + MAX_BATCH_SIZE + " or lower");
			}
		}
	}
