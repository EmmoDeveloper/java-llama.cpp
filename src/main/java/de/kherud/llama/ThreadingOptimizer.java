...
public class ThreadingOptimizer {
	private static final System.Logger logger = System.getLogger(ThreadingOptimizer.class.getName());
	...
	private static synchronized void detectSystemCapabilities() {
		...
		logger.log(DEBUG, "🔧 Threading: Detected %d cores, %d threads, NUMA: %s",
	...
	private static void applyNUMAOptimizations(ModelParameters params, CPUTopology topology) {
		...
		logger.log(DEBUG, "🔧 Threading: NUMA system detected - consider manual NUMA tuning");
	}
	...
	private static void logThreadingConfiguration(int threads, int batchThreads, CPUTopology topology) {
		logger.log(DEBUG, "🔧 Threading: Using %d threads for generation, %d for batches (detected %d cores)",
